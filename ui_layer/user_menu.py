...
"""
        ...
        if user_input == "1":
            try:
                ...
                result = user.add_expense(date, category, amount, description)
                if result:
            ...
            if data:
                ...
                try:
                    expense_id = int(
                        input("Enter a valid expense id to update expense")
                    )
                    result = user.update_expense(expense_id)
                    if result:
            ...
            if data:
                ...
                try:
                    expense_id = int(
                        input("Enter a valid id of an expense to be deleted").strip()
                    )
                    user.delete_expense(expense_id)
                except ValueError:
            ...
            filters = {
                "transport": input(
                    'Enter "y" to show transport expenses or Press Enter to skip'
                ).strip(),
                "housing": input(
                    'Enter "y" to show housing expenses or Press Enter to skip'
                ).strip(),
                "food": input(
                    'Enter "y" to show food expenses or Press Enter to skip'
                ).strip(),
                "clothing": input(
                    'Enter "y" to show clothing expenses or Press Enter to skip'
                ).strip(),
                "other": input(
                    'Enter "y" to show other expenses or Press Enter to skip'
                ).strip(),
            }
            filters = {key: value for key, value in filters.items() if value == "y"}
            user.show_expense_by_category(filters)

        elif user_input == "6":
            user.set_budget_by_category()

        elif user_input == "7":
            data = user.show_budget_status_by_category()
            if data:
