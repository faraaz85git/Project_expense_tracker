...
class TestAdmin:
    ...
    def test_show_all_user1_has_user(self, monkeypatch):
        self.mock_db_manager.fetch_data = MagicMock(
            return_value=[("1", "user1", "user")]
        )

        # act
        monkeypatch.setattr(
            "business_layer.user.database_manager", lambda: self.mock_db_manager
        )
    ...
    def test_delete_user1_valid_id_deleted(self, monkeypatch):
        ...
        monkeypatch.setattr("business_layer.user.Expense", lambda: self.mock_expense)
        monkeypatch.setattr(
            "business_layer.Admin.Admin.show_all_user1", mock_show_all_user1
        )
        admin = Admin("Raj", "sddsa", "admin")
        admin.delete_user1()
    ...
    def test_show_all_user_expenses1_has_data(self, monkeypatch):
        # arrange
        self.mock_db_manager.fetch_data = MagicMock(
            return_value=[("1", "user1", "2024-08-01", "housing", 1222, "")]
        )
