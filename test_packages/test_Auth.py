...
class TestAuth:
    ...
    def test_login1_valid_credentials(self, monkeypatch):
        self.db_manager.fetch_data = MagicMock(
            return_value=[("username", "password", "user")]
        )
        self.mock_bcrypt = MagicMock(return_value=True)

        monkeypatch.setattr(
            "business_layer.Auth.database_manager", lambda: self.db_manager
        )
    ...
    def test_login1_valid_username_invalid_password(self, monkeypatch):
        self.db_manager.fetch_data = MagicMock(
            return_value=[("valid_username", "valid_password", "user")]
        )
    ...
    def test_sign_up1_username_exists(self, monkeypatch):
        self.db_manager.fetch_data = MagicMock(
            return_value=[("user_id", "username", "password", "role")]
        )
        monkeypatch.setattr(
            "business_layer.Auth.database_manager", lambda: self.db_manager
        )
