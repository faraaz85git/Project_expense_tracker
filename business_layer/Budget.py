...
class Budget:
    ...
    def active_budget(self, username):
        try:
            ...
            active_budget = self.db_manager.fetch_data(
    ...
    def latest_budget(self, username):
        try:
            ...
            prev_budget = self.db_manager.fetch_data(
                table_name=table_name,
                columns=columns,
    ...
    def set_budget(self, username, prev_budget_end_date, entries):
        ...
        try:
            if prev_budget_end_date:
                ...
                if prev_budget_end_date >= entries.get("start_date"):
                    ...
                    raise BudgetNotSetException(
                        f"Budget is not set as start_date {entries["start_date"]} "
            ...
            result = self.db_manager.insert_data(
                table_name=table_name, columns=column, values=value
            )
            # if result:
            #      print('-------Budget is set.--------')
            # else:
