...
class User:
    def __init__(self, logger, username, password=None, role="user"):
        self.username = username
        self.password = password
        self.role = role
        self.expense = Expense(logger=logger)
    ...
    def add_expense(self, date, category, amount, description):
        ...
        try:
            result = self.expense.add_expense(
    ...
    def show_all_expense(self):
        try:
            ...
            raise
    def show_all_expense_pagination(self,limit,offset):
        try:

            data = self.expense.get_expense_pagination(self.username,limit,offset)
            return data
        except Exception:
            raise
    ...
    def show_expense_by_category(self, filters):
        try:
            if not filters:
                ...
                categories=["category=?" for filter in filters]
                categories=f"({(" OR ").join(categories)})"
                condition = [" username=? ",categories]
                parameter = [self.username]+filters
            ...
            raise
    def show_expense_by_category_pagination(self, filters,limit,offset):
        try:
            if not filters:
                # print('No category is provided.')
                raise InvalidCategoryException()
                ...
                condition = [" username=? ",categories]
                parameter = [self.username]+filters
                data = self.db_manager.fetch_data_paginated(
                    table_name=table_name,
                    columns=columns,
                    where_clause=condition,
                    parameters=parameter,
    ...
    def show_budget_status_by_category(self):
        try:
            self.logger.log(message="Fetching active or latest budget.")
            budget = Budget(logger=self.logger)
            latest_budget = budget.latest_budget(self.username)
            if latest_budget:
                self.logger.log(message="Latest budget is fetched successfully.")
                ...
                expense_data = self.show_all_expense()
                for data in expense_data:
                    if data[1] >= start_date and data[1] <= end_date:
                        amount_spend[data[2]] += data[3]

                self.logger.log(message="Generating budget status.")
                budget_status = {
                ...
                self.logger.log(message="Generated budget status.")
                return [
                    budget_amount,
                    amount_spend,
                    budget_status,
                ...
                self.logger.log(message="No budget to show budget status.")
    ...
    def plot_expense1(self):
        ...
        if data:
            ...
            category_expense = {
                key: value for key, value in category_expense.items() if value
            }
    ...
    def set_budget_by_category(self, new_budget):
        print(new_budget)
        try:
            budget = Budget(logger=self.logger)
            ...
            if active_budget:
                ...
                raise BudgetNotSetException(
                    f"Budget is not set as you already have active budget from {start_date} to {end_date}"
                )
                ...
                if prev_budget:
                    ...
                    budget.set_budget(
                        self.username,
                        prev_budget_end_date=prev_budget_end_date,
                        entries=new_budget,
                    )
                    ...
                    budget.set_budget(
                        self.username, prev_budget_end_date=None, entries=new_budget
                    )
