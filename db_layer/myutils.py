...
def display(expense_data):
    print(
        f"{'expense_id':<15} {'date':<15} {'category':<15} {'amount':<15} {'description':<15}"
    )
    print("-" * 80)
    for data in expense_data:
        expense_id, date, category, amount, description = data
        print(
            f"{expense_id:<15} {date:<15} {category:<15} {amount:<15} {description:<100}"
        )
...
def display_budget_status(budget_amount, amount_spend):
    ...
    print("-" * 60)
    print(
        f"{'Housing':<9} {budget_amount['housing']:>15} {amount_spend['housing']:>15} {budget_status['housing']:>15}"
    )
    print(
        f"{'Transport':<9} {budget_amount['transport']:>15} {amount_spend['transport']:>15} {budget_status['transport']:>15}"
    )
    print(
        f"{'Food':<9} {budget_amount['food']:>15} {amount_spend['food']:>15} {budget_status['food']:>15}"
    )
    print(
        f"{'Clothing':<9} {budget_amount['clothing']:>15} {amount_spend['clothing']:>15} {budget_status['clothing']:>15}"
    )
    print(
        f"{'Other':<9} {budget_amount['other']:>15} {amount_spend['other']:>15} {budget_status['other']:>15}"
    )
...
def validate_budget_date(start_date, end_date):
    if end_date < start_date:
        raise BudgetNotSetException(
            f"Invalid budget start date {start_date} should be smaller than or "
