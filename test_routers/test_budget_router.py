...
active_budget = [(1000.0, 1000.0, 1000.0, 1000.0, 1000.0, "2024-10-16", "2024-10-16")]
...
class Test_budget:
    ...
    @pytest.mark.asyncio
    async def test_get_active_budget_has_active_budget_return_budget(self, monkeypatch):
        # arrange
        ...
        result = get_active_budget(self.mock_user)
    ...
    @pytest.mark.asyncio
    async def test_get_active_budget_no_active_budget_return_empty_list(self, monkeypatch):
        # arrange
        mock_budget = MagicMock()
        ...
        assert result.get("status") == "No active budget is found."
    @pytest.mark.asyncio
    async def test_get_active_budget_sqlite_exception(self, monkeypatch):
        ...
        with pytest.raises(HTTPException) as exe:
            get_active_budget(self.mock_user)
    ...
    @pytest.mark.asyncio
    async def test_create_budget_created_success(self, monkeypatch):
        # arrange
        mock_validate_budget_date = MagicMock(return_value=True)
        self.mock_user.set_budget_by_category = MagicMock()
        ...
        # act
        monkeypatch.setattr(
            "Router.Budget.validate_budget_date", mock_validate_budget_date
        )
        result = await set_budget(self.mock_user, mock_budget_create)

        # assert
        mock_validate_budget_date.assert_called_once_with(
            mock_budget_create.start_date, mock_budget_create.end_date
        )
        mock_budget_create.model_dump.assert_called_once()
        self.mock_user.set_budget_by_category.assert_called_once_with(
            new_budget={"username": "user"}
        )
        assert result.get("created") == "success"

    @pytest.mark.asyncio
    async def test_create_budget_end_date_smaller_than_start_date_bad_request(self, monkeypatch):
        # arrange
        mock_budget_create = MagicMock()
        ...
        assert (
            exc.value.detail
            == "Invalid budget start date 2024-10-10 should be smaller than or equal to end data 2024-10-09."
        )

    @pytest.mark.asyncio
    async def test_create_budget_overlap_with_prev_budget_bad_request(self, monkeypatch):
        # arrange
        mock_validate_budget_date = MagicMock(return_value=True)
        self.mock_user.set_budget_by_category = MagicMock(
            side_effect=BudgetNotSetException()
        )
        self.mock_user.username = "user"
        mock_budget_create = MagicMock()
        mock_budget_create.start_date = MagicMock()
    ...
    async def test_create_budget_not_general_exception(self, monkeypatch):
        # arrange
        mock_validate_budget_date = MagicMock(return_value=True)
        self.mock_user.set_budget_by_category = MagicMock(side_effect=Exception())
        self.mock_user.username = "user"
        mock_budget_create = MagicMock()
        mock_budget_create.start_date = MagicMock()
        mock_budget_create.end_date = MagicMock()
